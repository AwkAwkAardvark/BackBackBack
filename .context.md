...
* `fix(auth)` 로그인/재발급/회원가입 응답을 공통 ApiResponse로 정합화 및 refresh 무토큰 401 처리 보강
* `fix(auth)` 이메일 인증/재전송 성공·실패 응답을 공통 ApiResponse 포맷으로 통일
* `fix(company)` 기업 검색 API keyword/name 파라미터 호환 및 응답 필드 스키마 정합화
* `fix(report)` 보고서 PDF 다운로드 URL 조회 엔드포인트 추가 및 보안 규칙 보강
* `test(auth/company/report)` 관련 테스트 갱신 및 통과
...
* **2026-02-03**
    * 인증/기업검색/보고서 다운로드 응답 포맷 정합화 및 테스트 보정
